/// Class implementation is significantly slower than just using functions themselves.
EMPTY = null
PLP = 0
AIP = 1
TURN = PLP;
...
function new_game() {
    BOARD = Array(CONFIG.rows).fill(null).map(() => Array(CONFIG.columns).fill(null));
}
...
function play(board, col, piece) { /// drop_piece renamed
...
function evaluate_window(window, piece) {
    let score = 0, opponentPiece = PLP;
    if (piece === PLP) opponentPiece = AIP;
	let _numConnected = window.filter(p => p === piece).length, _numEmpty = window.filter(p => p === EMPTY).length, 
		_numOpponentConnected = window.filter(p => p === opponentPiece).length;
	
    // A win for that player/AI's piece. (By default, 4 in a row).
...
function is_terminal_node(board) {
    return winning_move(board, PLP) || winning_move(board, AIP) || get_valid_locations(board).length === 0;
}
...
// [ col, minimaxScore ] = minimax(BOARD, 11, -Infinity, Infinity, true);
module.exports = {
    new_game, display_board, set, set_pos, play, winning_move, evaluate_window, score_position, is_terminal_node, minimax, get_valid_locations
};